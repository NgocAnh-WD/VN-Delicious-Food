    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="post" enctype='multipart/form-data'>
        ...
        <div class=" form-group {{ $errors->has('category_id') ? 'has-error' : '' }}">
            ...
            <select class="form-control" id="category_id" name="category_id" >
                ...
                <option value="{{$id}}" {{ $id == $product->category->id ? 'selected="selected"' : '' }}>{{$name}}</option>
                @endforeach   
            </select>                
        ...
        <div class="row" id="show_image_delete">
            ...
            <div class="col-md-3">
                <div class="form-group" style="width: 200px">
                    ...
                    <div class="space-4"></div>    
                    <button type="button" class="" id="image_delete{{$img->id}}" onclick="deleteImages({{$img->id}})" value="{{$img->id}}">Delete</button>    
                </div>                   
            </div>             
            @endforeach
        </div>      
